...
get.aes.for.ggplot <- function(roc, legacy.axes, legend_name = "name", extra_aes = c()) {
...
ggroc.roc <- function(data, legacy.axes = FALSE, ...) {
  ...
  # Prepare the aesthetics
  aes <- get.aes.for.ggplot(data, legacy.axes, legend_name = "ROC Curves")
...
ggroc.list <- function(data, aes = c("colour", "alpha", "linetype", "size", "group"), legacy.axes = FALSE, ...) {
  ...
  for (i in seq_along(coord.dfs)) {
    coord.dfs[[i]]$Folds <- names(coord.dfs)[i]
  }
  
  # Make a big data.frame
  coord.dfs <- do.call(rbind, coord.dfs)
  coord.dfs$Folds <- factor(coord.dfs$Folds, as.vector(names(data)))
  
  # Prepare the aesthetics
  aes.ggplot <- get.aes.for.ggplot(data[[1]], legacy.axes, legend_name="Folds", aes)
